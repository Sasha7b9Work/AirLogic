...
FRAMEWORK_DIR = platform.get_package_dir("framework-ststm32")
CMSIS_DIR = join(platform.get_package_dir("framework-ststm32"), "CMSIS", "CMSIS")
...
def process_standard_library_configuration(cpp_defines):
    if "PIO_FRAMEWORK_STM32_STANDARD_LIB" in cpp_defines:
        env["LINKFLAGS"].remove("--specs=nano.specs")
    if "PIO_FRAMEWORK_STM32_NANOLIB_FLOAT_PRINTF" in cpp_defines:
        env.Append(LINKFLAGS=["-u_printf_float"])
    if "PIO_FRAMEWORK_STM32_NANOLIB_FLOAT_SCANF" in cpp_defines:
        env.Append(LINKFLAGS=["-u_scanf_float"])


def process_usart_configuration(cpp_defines):
    if "PIO_FRAMEWORK_STM32_SERIAL_DISABLED" in cpp_defines:
        env["CPPDEFINES"].remove("HAL_UART_MODULE_ENABLED")

    elif "PIO_FRAMEWORK_STM32_SERIAL_WITHOUT_GENERIC" in cpp_defines:
        env.Append(CPPDEFINES=["HWSERIAL_NONE"])


def process_usb_speed_configuration(cpp_defines):
    if "PIO_FRAMEWORK_STM32_USB_HIGHSPEED" in cpp_defines:
        env.Append(CPPDEFINES=["USE_USB_HS"])

    elif "PIO_FRAMEWORK_STM32_USB_HIGHSPEED_FULLMODE" in cpp_defines:
        env.Append(CPPDEFINES=["USE_USB_HS", "USE_USB_HS_IN_FS"])


def process_usb_configuration(cpp_defines):
    if "PIO_FRAMEWORK_STM32_ENABLE_CDC" in cpp_defines:
        env.Append(CPPDEFINES=["USBD_USE_CDC"])

    elif "PIO_FRAMEWORK_STM32_ENABLE_CDC_WITHOUT_SERIAL" in cpp_defines:
        env.Append(CPPDEFINES=["USBD_USE_CDC", "DISABLE_GENERIC_SERIALUSB"])

    elif "PIO_FRAMEWORK_STM32_ENABLE_HID" in cpp_defines:
...
env.Append(
    ...
    CPPPATH=[
        join(FRAMEWORK_DIR, "cores", "stm", "avr"),
        join(FRAMEWORK_DIR, "cores", "stm", "stm32"),
        join(FRAMEWORK_DIR, "cores", "stm", "stm32", "LL"),
        join(FRAMEWORK_DIR, "cores", "stm", "stm32", "usb"),
        join(FRAMEWORK_DIR, "cores", "stm", "stm32", "usb", "hid"),
        join(FRAMEWORK_DIR, "cores", "stm", "stm32", "usb", "cdc"),
...
if "build.variant" in env.BoardConfig():
    ...
    env.BuildSources(join("$BUILD_DIR", "FrameworkVariant"), variant_dir)

env.BuildSources(
    join("$BUILD_DIR", "Framework"), join(FRAMEWORK_DIR, "cores", "stm")
)

env.BuildSources(
    join("$BUILD_DIR", "Src"), join(FRAMEWORK_DIR, "libraries", "SrcWrapper")
)
