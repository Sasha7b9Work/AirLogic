...
#ifdef LCD_EXT
#define deviceName "alpro_v10_lcdext"
static const uint8_t deviceId = MF_ALPRO_V10_LCD_EXT;
MFPROTOPHYSERIAL(SlavePhy, Serial1, PA12,
                 BAUD / 2); // Serial physical layer
#else
#define deviceName "alpro_v10_lcd"
...
#ifdef LCD_EXT
#define _INT_FLASH alpro_v10_lcdext_spi
MFBUS(_INT_FLASH, spiproto, MF_ALPRO_V10_LCD_EXT_SPI,
      MFBUS_CAP_DEBUG | MFBUS_CAP_BOOTLOADER);

#else
#define _INT_FLASH alpro_v10_lcd_spi
MFBUS(_INT_FLASH, spiproto, MF_ALPRO_V10_LCD_SPI,
      MFBUS_CAP_DEBUG | MFBUS_CAP_BOOTLOADER);
...
/* bus masters */
MFPROTOPHYSERIAL(COM1, Serial1, MFPROTO_PHY_SERIAL_NC, BAUD / 2);
    ...
    case MFBUS_BL_WRITE: {
      ...
      if (msg->head.size >= sizeof(MFBUS_FLASH_PACKET_HEADER)) {
        if (__HAL_FLASH_GET_FLAG(FLASH_FLAG_BSY) != RESET) {
          ...
          if (msg->head.size ==
              sizeof(MFBUS_FLASH_PACKET_HEADER) + head->data.data_size) {
            /* deny writing to the bootloader area */
            if (head->addr < FLASH_BASE + BL_OFFSET)
              head->addr = FLASH_BASE + BL_OFFSET;
            /* check the crc32 */
            uint32_t crc = mfCRC::crc32(
                (uint32_t *)&msg->data[sizeof(MFBUS_FLASH_PACKET_HEADER)],
                head->data.data_size);
...
/* poll buses for new messages */
...
#ifdef BL_F401
  /* init lcd to show "firmware upgrade" string */
  SPI.setMOSI(_MOSI);
  SPI.setMISO(_MISO);
  SPI.setSCLK(_SCLK);
  SPI.setSSEL(_CS);
  ...
  mylcd2.init();
  mylcd2.setFont(&fixed66pt8b);
  String loading = "[ firmware upgrade ]";
  mylcd2.rectangle(10, 10, mylcd2.width - 10, mylcd2.height - 10,
                   RGB(31, 63, 31));
  mylcd2.printFast((mylcd2.width - mylcd2.printWidth(loading)) / 2,
                   (mylcd2.height / 2) + 4, loading, RGB(31, 63, 31),
                   RGB(0, 0, 0));
#endif
}
