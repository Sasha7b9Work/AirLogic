...
#define LL_BUT BL_BUTPORT, BL_BUTPIN
/* programming led to use at early start */
#define LL_LED BL_LEDPORT, BL_LEDPIN
  _enableGPIOClock(BL_BUTPORT); // button clock
  LL_GPIO_SetPinMode(LL_BUT, LL_GPIO_MODE_INPUT);
  LL_GPIO_SetPinPull(LL_BUT, LL_GPIO_PULL_UP);
  LL_GPIO_SetPinSpeed(LL_BUT, LL_GPIO_SPEED_FREQ_HIGH);
  ...
  uint8_t readValue = LL_GPIO_IsInputPinSet(LL_BUT);
  /*#ifdef BL_F401
    readValue = 0;
  #endif*/
