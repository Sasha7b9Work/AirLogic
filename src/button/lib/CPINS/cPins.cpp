...
cPins::cPins(const char *cname, uint16_t _PIN, uint8_t isled, uint8_t hs)
    : port(get_GPIO_Port(STM_PORT(digitalPinToPinName(_PIN)))),
      ...
      isHW(isled & 2U), isLed(isled & 1U), highState(hs & 1U),
      lowState(!(hs & 1U)) {
  uint8_t isOD = hs & 2U;
...
__attribute__((optimize(3))) inline void cPins::write(uint8_t value) {
  port->BSRR = (pin) << (!(value & 1U) << 4);
}
// only to read output ports, input ports should be read from IDR
__attribute__((optimize(3))) inline uint8_t cPins::read(void) {
  return port->ODR & (pin);
}
__attribute__((optimize(3))) inline void cPins::reset(void) {
  write(lowState);
}
__attribute__((optimize(3))) inline void cPins::set(void) {
  write(highState);
}
__attribute__((optimize(3))) inline void cPins::toggle(void) {
  port->ODR ^= (pin);
}
...
__attribute__((optimize(3))) void cPins::timerCallback(void) {
      ...
      if (instances[c]->blinkTime == 0) {
        if ((instances[c]->isHW) && (instances[c]->prevDuty > 0)) {
          ...
          pwm_start(digitalPinToPinName(instances[c]->pinHW),
                    instances[c]->pinFrequency,
                    instances[c]->lowState ? PWMMAX - 1 : 0,
