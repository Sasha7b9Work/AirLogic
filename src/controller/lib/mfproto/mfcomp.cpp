...
void mfCompControl::pumpCallback(cPins *pin) {
  ...
  if (settings.externalValve)
    clearFlag(MF_CC_VALVE_REQUEST);
  pumpMutex.clear();
}
...
uint16_t mfCompControl::getPSI(float voltage) {
  if (voltage < MIN_SENSOR_VOLTAGE)
    voltage = MIN_SENSOR_VOLTAGE;
  if (voltage > MAX_SENSOR_VOLTAGE)
    voltage = MAX_SENSOR_VOLTAGE;
  float newPSI = (settings.maxPSI / (MAX_SENSOR_VOLTAGE - MIN_SENSOR_VOLTAGE)) *
                 (voltage - MIN_SENSOR_VOLTAGE);
...
bool mfCompControl::isPumpLockedNotExt() {
  bool locked = false;
  for (int types = 0; types < LOCK_END; types++) {
    if ((pumpLocks[types].lock) &&
        ((HAL_GetTick() - pumpLocks[types].moment) >= pumpLocks[types].time))
      unlockPump((MF_CC_LOCK_TYPE)types);
    if (pumpLocks[types].lock && types != LOCK_EXT_VALVE)
      locked = true;
  }
  return locked;
}
...
void mfCompControl::startCompressor(uint32_t time) {
  if (pump) {
    if (settings.externalValve && pumpLocks[LOCK_EXT_VALVE].lock) {
      setFlag(MF_CC_VALVE_REQUEST);
      return;
    }
    if (!isPumpLocked()) {
...
void mfCompControl::pause() {
  if (currentState.saved)
    return;
  currentState.state = state.load();

  for (int types = 0; types < LOCK_END; types++) {
    currentState.pumpLocks[types] = pumpLocks[types];
    unlockPump((MF_CC_LOCK_TYPE)types);
  }
  currentState.compLockCounter = compLockCounter;
  currentState.compOnTime = pump->getRemaining();
  if (currentState.compOnTime)
    pump->off();

  currentState.dryLockCounter = dryLockCounter;
  currentState.dryLock = dryLock;
  unlockDryer();
  currentState.dryOnTime = dry->getRemaining();
  if (currentState.dryOnTime)
    dry->off();

  state.store(0UL);
  currentState.saved = true;
}
void mfCompControl::resume() {
  if (!currentState.saved)
    return;
  currentState.saved = false;

  state.store(currentState.state);

  for (int types = 0; types < LOCK_END; types++) {
    pumpLocks[types] = currentState.pumpLocks[types];
  }
  compLockCounter = currentState.compLockCounter;
  if (currentState.compOnTime)
    pump->on(currentState.compOnTime);

  dryLockCounter = currentState.dryLockCounter;
  dryLock = currentState.dryLock;
  if (currentState.dryOnTime)
    dry->on(currentState.dryOnTime);
}
...
uint8_t mfCompControl::busAnswerCallback(mfBus *instance, MF_MESSAGE *msg,
  ...
  case MFBUS_CC_ENABLE: {
    ...
    if (msg->head.size == sizeof(uint8_t)) {
      ...
      if (!param) {
        ...
        if (checkFlag(MF_CC_PUMP_ON))
          stopCompressor();
        if (checkFlag(MF_CC_DRY_ON))
          stopDryer();
      } else
  ...
  case MFBUS_CC_SETTINGS: {
    if (msg->head.size == sizeof(MF_CC_SETTINGS)) {
      ...
      if (settings.externalValve) {
        lockPump(LOCK_EXT_VALVE, ~0UL);
      } else {
        unlockPump(LOCK_EXT_VALVE);
      }
      clearFlag(MF_CC_VALVE_REQUEST);
    }
  ...
  case MFBUS_CC_GET_TEMP: {
    float curTemp = getTempF();
    if (::fabsf(lastTemp - curTemp) > 0.5)
      lastTemp = curTemp;
    int32_t temp = lrint(lastTemp);
  ...
  case MFBUS_CC_LOCK_COMP: {
    if (!compLockCounter)
      lockPump(LOCK_EXT, ~0UL); // infinite atm
    compLockCounter++;
    stopCompressor();
    instance->sendMessage(newmsg, NULL, 0);
  } break;
  ...
  case MFBUS_CC_IS_COMP_LOCKED: {
    uint8_t flag = (uint8_t)isPumpLockedNotExt();
    instance->sendMessage(newmsg, &flag, sizeof(flag));
  } break;
  ...
  case MFBUS_CC_LOCK_COMP_EXT: {
    lockPump(LOCK_EXT_VALVE, ~0UL); // infinite atm
    stopCompressor();
    instance->sendMessage(newmsg, NULL, 0);
  } break;
  case MFBUS_CC_UNLOCK_COMP_EXT: {
    unlockPump(LOCK_EXT_VALVE);
    instance->sendMessage(newmsg, NULL, 0);
  } break;
  case MFBUS_CC_PAUSE: {
  ...
  case MFBUS_CC_RESUME: {
    resume();
    instance->sendMessage(newmsg, NULL, 0);
  } break;
  case MFBUS_CC_VALVE_REQUEST: {
    uint8_t flag = (uint8_t)checkFlag(MF_CC_VALVE_REQUEST);
    instance->sendMessage(newmsg, &flag, sizeof(flag));
  } break;
  case MFBUS_CC_IS_COMP_EXT_LOCKED: {
    uint8_t flag = (uint8_t)pumpLocks[LOCK_EXT_VALVE].lock;
    instance->sendMessage(newmsg, &flag, sizeof(flag));
  } break;
...
void mfBusCompControl::denyExtValve() {
  uint32_t id = bus.sendMessageWait(device, MFBUS_CC_LOCK_COMP_EXT, NULL, 0);
  if (id)
    bus.deleteMessageId(id);
}
void mfBusCompControl::allowExtValve() {
  // entriesComp = 0;
  uint32_t id = bus.sendMessageWait(device, MFBUS_CC_UNLOCK_COMP_EXT, NULL, 0);
  if (id)
    bus.deleteMessageId(id);
}
bool mfBusCompControl::isValveRequested() {
  uint8_t *answer;
  bool ret = false;
  uint32_t id = bus.sendMessageWait(device, MFBUS_CC_VALVE_REQUEST, NULL, 0);
  if (id) {
    MF_MESSAGE msg;
    bus.readMessageId(msg, id);
    if (msg.head.id && (msg.head.size == sizeof(uint8_t))) {
...
bool mfBusCompControl::isExtLocked() {
  uint8_t *answer;
  bool ret = false;
  uint32_t id =
      bus.sendMessageWait(device, MFBUS_CC_IS_COMP_EXT_LOCKED, NULL, 0);
  if (id) {
    MF_MESSAGE msg;
    bus.readMessageId(msg, id);
    if (msg.head.id && (msg.head.size == sizeof(uint8_t))) {
...
void mfBusCompControl::pause() {
  uint32_t id = bus.sendMessageWait(device, MFBUS_CC_PAUSE, NULL, 0);
  if (id)
    bus.deleteMessageId(id);
}
void mfBusCompControl::resume() {
  uint32_t id = bus.sendMessageWait(device, MFBUS_CC_RESUME, NULL, 0);
  if (id)
    bus.deleteMessageId(id);
}
