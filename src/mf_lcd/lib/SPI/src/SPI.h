...
/*
 * Defines the number of settings saved per SPI instance. Must be in range 1 to
 * 254. Can be redefined in variant.h
 */
...
public:
  SPISettings(uint32_t clock, BitOrder bitOrder, uint8_t dataMode,
              bool noRecv = SPI_TRANSMITRECEIVE) {
    clk = clock;
    bOrder = bitOrder;
    noReceive = noRecv;
...
public:
  SPIClass();
  SPIClass(uint8_t mosi, uint8_t miso, uint8_t sclk,
           uint8_t ssel = (uint8_t)NC);
  ...
  void endTransaction(void) { endTransaction(CS_PIN_CONTROLLED_BY_USER); }
  ...
  void transfer(uint8_t pin, void *_buf, size_t _count,
                SPITransferMode _mode = SPI_LAST);
  void transfer(byte _pin, void *_bufout, void *_bufin, size_t _count,
                SPITransferMode _mode = SPI_LAST);
...
private:
  /* Contains various spiSettings for the same spi instance. Each spi
  spiSettings is associated to a CS pin. */
  SPISettings spiSettings[NB_SPI_SETTINGS];

  // Use to know which configuration is selected.
  int16_t _CSPinConfig;

  typedef enum { GET_IDX = 0, ADD_NEW_PIN = 1 } pin_option_t;
