...
MFBUS(SlaveBus, SlaveProto, MF_ALPRO_V10_LCD, MFBUS_CAP_DEBUG); // bus instance
#include "debug.h"
...
#endif
const GFXfont *MFLCD_availableFonts[3]{&fixed66pt8b, &fixedsys8pt8b,
                                       &Roboto_Bold12pt8b};

CPIN(BL, PB8, CPIN_LED, CPIN_HIGH);
CPIN(GR, PB7, CPIN_LED, CPIN_HIGH);
CPIN(RD, PB6, CPIN_LED, CPIN_HIGH);
...
volatile uint16_t refreshMutex = 0;
...
void refreshCallback(void) {
  if (refreshMutex || messagesArrived)
    return;
  refreshMutex++;
...
void setup() {
  ...
  refreshTimer.setOverflow(24, HERTZ_FORMAT);
