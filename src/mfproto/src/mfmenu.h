...
mfSettings set;
MF_SETTINGS &dSet = set.data;
MF_LANGUAGE &LANG = set.data.language;
...
auto languageItem =
    new mfMenuItem<MF_LANGUAGE>(TR_language, &dSet.language,
...
auto axlesItem = new mfMenuItem<axleType>(TR_axles, &dSet.axles,
...
auto receiverItem = new mfMenuItem<bool>(TR_receiver, &dSet.receiverPresent,
...
auto compOnPressureItem = new mfMenuItem<uint16_t>(
    TR_compOnPressure, &dSet.compOnPressure, MF_COMP_ON_START, MF_COMP_ON_END,
    MF_COMP_ONOFF_STEP, "%uPSI");

auto compOffPressureItem = new mfMenuItem<uint16_t>(
    TR_compOffPressure, &dSet.compOffPressure, MF_COMP_OFF_START,
    MF_COMP_OFF_END, MF_COMP_ONOFF_STEP, "%uPSI");

auto compEnableItem = new mfMenuItem<uint16_t>(
    TR_compEnable, &dSet.compEnablePressure, 0, 100, 10, "%uPSI");

auto compTempSensorItem = new mfMenuItem<uint16_t>(
    TR_compTempSensor, &dSet.compTempSensor, 50, 200, 10, "%u\xFС");
/**************************************************************************/
...
auto dryerItem = new mfMenuItem<dryerType>(TR_dryer, &dSet.dryer,
                                           {{MF_DRYER_NO, TR_no},
...
auto exhaustTimeItem = new mfMenuItem<uint8_t>(
    TR_exhaustTime, &dSet.dryerExhaustTime, 0, 10, 1, "%u sec");
/**************************************************************************/
MF_TRANSLATION TR_speedSensEnabled = {"Speed control", "Модуль скорости"};
auto speedSensItem =
    new mfMenuItem<bool>(TR_speedSensEnabled, &dSet.speedSensEnabled,
                         {{false, TR_no}, {true, TR_present}});
...
auto speedSensLimit1Item = new mfMenuItem<uint16_t>(
    TR_speedSensLimit1, &dSet.speedSensLimit1, 10, 100, 10, "%u km/h");
auto speedSensLimit2Item = new mfMenuItem<uint16_t>(
    TR_speedSensLimit2, &dSet.speedSensLimit2, 10, 100, 10, "%u km/h");
auto speedSensLimit2presetItem = new mfMenuItem<uint8_t>(
    TR_speedSensLimit2preset, &dSet.speedSensLimit2preset, 20, 80, 10, "%u%%");
...
MF_TRANSLATION TR_engineSensEnabled = {"Engine start sense",
                                       "Контроль двигателя"};
auto engineSensItem =
    new mfMenuItem<bool>(TR_engineSensEnabled, &dSet.engineSensEnabled,
                         {{false, TR_no}, {true, TR_yes}});
/**************************************************************************/
MF_TRANSLATION TR_autoHeightEnabled = {"Height on start", "Подъем при старте"};
auto autoHeightItem =
    new mfMenuItem<bool>(TR_autoHeightEnabled, &dSet.autoHeightEnabled,
                         {{false, TR_no}, {true, TR_yes}});

MF_TRANSLATION TR_autoHeightCorrection = {"Auto height tracking",
                                          "Автокоррекция клиренса"};
auto autoHeightCorrectionItem =
    new mfMenuItem<bool>(TR_autoHeightCorrection, &dSet.autoHeightCorrection,
                         {{false, TR_no}, {true, TR_yes}});
...
auto buttonManualModeItem =
    new mfMenuItem<bool>(TR_buttonManualMode, &dSet.buttonManualMode,
...
auto sensitivityItem = new mfMenuItem<uint8_t>(
    TR_sensitivity, &dSet.sensitivity,
    {{2, TR_senshigh}, {1, TR_sensmedium}, {0, TR_senslow}});
/**************************************************************************/
MF_TRANSLATION TR_delay = {"Delay", "Задержка"};
auto delayItem =
    new mfMenuItem<uint8_t>(TR_delay, &dSet.delay, 3, 10, 1, "%u sec");
/**************************************************************************/
MF_TRANSLATION TR_wifiEnabled = {"WIFI"};
auto wifiEnabledItem = new mfMenuItem<bool>(TR_wifiEnabled, &dSet.wifiEnabled,
                                            {{false, TR_no}, {true, TR_yes}});
/**************************************************************************/
MF_TRANSLATION TR_btEnabled = {"Bluetooth"};
auto btEnabledItem = new mfMenuItem<bool>(TR_btEnabled, &dSet.btEnabled,
                                          {{false, TR_no}, {true, TR_yes}});
/**************************************************************************/
MF_TRANSLATION TR_compressorEnabled = {"Compressor", "Компрессор"};
auto compressorEnabledItem =
    new mfMenuItem<bool>(TR_compressorEnabled, &dSet.compressorEnabled,
                         {{false, TR_no}, {true, TR_present}});
/**************************************************************************/
MF_TRANSLATION TR_speedLock = {"Compressor lock", "Блокировка компрессора"};
auto speedLockItem = new mfMenuItem<uint8_t>(TR_speedLock, &dSet.speedLock, 10,
                                             100, 10, "%u km/h");
/**************************************************************************/
MF_TRANSLATION TR_voltageCorrection = {"Voltage correction",
                                       "Калибровка напряжения"};
auto voltageCorrectionItem = new mfMenuItem<double>(
    TR_voltageCorrection, &dSet.voltageCorrection, -1.0f, 1.0f, 0.1f, "%.1fV");
/**************************************************************************/
...
auto preset1Item =
    new mfMenuItem<uint8_t>(TR_preset1, &dSet.presets[0], 0, 100, 10, "%u%%");
auto preset2Item =
    new mfMenuItem<uint8_t>(TR_preset2, &dSet.presets[1], 0, 100, 10, "%u%%");
auto preset3Item =
    new mfMenuItem<uint8_t>(TR_preset3, &dSet.presets[2], 0, 100, 10, "%u%%");
auto preset4Item =
    new mfMenuItem<uint8_t>(TR_preset4, &dSet.presets[3], 0, 100, 10, "%u%%");
auto preset5Item =
    new mfMenuItem<uint8_t>(TR_preset5, &dSet.presets[4], 0, 100, 10, "%u%%");
/**************************************************************************/
MF_TRANSLATION TR_save = {"Save settings", "Сохранить настройки"};
auto saveCall = new mfMenuCall(TR_save, [](mfMenuCall *call) { set.save(); });

MF_TRANSLATION TR_resetSettings = {"Reset settings", "Сбросить настройки"};
auto resetCall =
    new mfMenuCall(TR_resetSettings, [](mfMenuCall *call) { set.reset(); });
...
void applyItemRules(void) {
  ...
  valvesItem34->disabledCheck = []() {
    ...
    return (dSet.axles == MF_ALL_AXLES);
  };
  ...
  calFrontCall->disabledCheck = []() { return (dSet.axles != MF_FRONT_AXLE); };
  calRearCall->disabledCheck = []() { return (dSet.axles != MF_REAR_AXLE); };

  speedSensLimit1Item->disabledCheck = []() {
    return (!dSet.speedSensEnabled);
  };
  ...
  axlesItem->onSet = [](mfMenuItemGen *) {
    ...
    uint8_t *p_valves = (uint8_t *)&dSet.valves;
    if (dSet.axles == MF_ALL_AXLES)
  ...
  compOffPressureItem->onSet = [](mfMenuItemGen *) {
    uint16_t topOnmagrin = (dSet.compOffPressure > MF_COMP_ON_END)
                               ? MF_COMP_ON_END
                               : dSet.compOffPressure - MF_COMP_ONOFF_STEP;
    compOnPressureItem->clear();
    compOnPressureItem->initValues(MF_COMP_ON_START, topOnmagrin,
                                   MF_COMP_ONOFF_STEP);
    if (dSet.compOnPressure > topOnmagrin)
      dSet.compOnPressure = topOnmagrin;
  };
  compOnPressureItem->onSet = [](mfMenuItemGen *) {
    uint16_t bottomOffmagrin = (dSet.compOnPressure < MF_COMP_OFF_START)
                                   ? MF_COMP_OFF_START
                                   : dSet.compOnPressure + MF_COMP_ONOFF_STEP;
    compOffPressureItem->clear();
    compOffPressureItem->initValues(bottomOffmagrin, MF_COMP_OFF_END,
                                    MF_COMP_ONOFF_STEP);
    if (dSet.compOffPressure < bottomOffmagrin)
      dSet.compOffPressure = bottomOffmagrin;
  };
  ...
  buttonUpDownColorItem->onSet = [](mfMenuItemGen *) {
    uint8_t r, g, b;
    r = (dSet.buttonOkColor & 0xFF0000) >> 16;
    g = (dSet.buttonOkColor & 0xFF00) >> 8;
    b = (dSet.buttonOkColor & 0xFF);
    /* not ready yes*/
    // set color up (rgb)
    // set color down (rgb)
  };
  ...
  buttonBrightnessItem->onSet = [](mfMenuItemGen *) {
    ledOK.setBrightness(dSet.buttonBrightness);
    // up .setBrightness(dSet.buttonBrightness);
    // down .setBrightness(dSet.buttonBrightness);
  };
  /* run em first time */
...
enum lcdButtons { LCD_UP = 0, LCD_OK = 1, LCD_DOWN = 2 };
