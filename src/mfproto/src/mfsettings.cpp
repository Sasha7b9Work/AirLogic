...
bool mfSettings::begin(SPIClass &SPI, uint32_t CSPin) {
  ...
  SPI.beginTransaction(
      SPISettings(SPI_CLOCK_DIV4, MSBFIRST, SPI_MODE0, SPI_TRANSMITRECEIVE));
  /* init spi flash */
...
void mfSettings::load(std::function<void()> loadCallback) {
  ...
  for (uint16_t sec = 0; sec < sectors; sec++) { // check every copy
    ...
    crc32 = mfCRC::crc32((uint32_t *)(&data + sizeof(data.crc32)),
                         (uint16_t)(sizeof(MF_SETTINGS) - sizeof(data.crc32)));
...
void mfSettings::save(std::function<void()> saveCallback) {
  ...
  data.crc32 = mfCRC::crc32((uint32_t *)(&data + sizeof(data.crc32)),
