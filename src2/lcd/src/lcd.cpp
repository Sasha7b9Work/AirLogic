...
void LCD::line(uint16_t x, uint16_t y, uint16_t x2, uint16_t y2,
  ...
  if (shortLen == 0)
    ...
    divDiff = (float)longLen / (float)shortLen;
  if (yLonger) {
    for (int i = 0; i != longLen; i += incrementVal) {
      point(x + (int)((float)i / divDiff), y + i, color);
    }
  } else {
    for (int i = 0; i != longLen; i += incrementVal) {
      point(x + i, y + (int)((float)i / divDiff), color);
    }
  }
}
...
void LCD::b_line(uint16_t x, uint16_t y, uint16_t x2, uint16_t y2,
  ...
  if (yLonger) {
    for (int i = 0; i != longLen; i += incrementVal) {
      b_point(x + (int)((float)i / divDiff), y + i, color);
    }
  } else {
    for (int i = 0; i != longLen; i += incrementVal) {
      b_point(x + i, y + (int)((float)i / divDiff), color);
    }
  }
}
...
void LCD::b_printRight(uint16_t x, uint16_t y, String &s, uint16_t color,
  ...
  int8_t npos = 0;
  do {
    npos = temp.lastIndexOf('\n');
    if (npos != -1) {
      String temp2 = temp.substring(npos + 1);
      len[lines] = LCD::printWidth(temp2);
      temp.remove(npos);
    } else {
      len[lines] = LCD::printWidth(temp);
    }
    maxlen = max(maxlen, len[lines]);
    lines++;
  } while (npos != -1);
  uint8_t curLine = lines - 1;
  uint16_t pos = x - len[curLine] * scale;
  for (uint16_t c = 0; c < s.length(); c++) {
    char curChar = s.charAt(c);
    if (curChar == '\n') {
      y += gfxFont->yAdvance;
      curLine--;
      pos = x - len[curLine] * scale;
      continue;
    }
    b_drawChar(pos, y, curChar, color, bg, scale, scale);
    GFXglyph *glyph = gfxFont->glyph + curChar - gfxFont->first;
...
void LCD::b_printCenter(uint16_t x, uint16_t w, uint16_t y, String &s,
  ...
  uint16_t pos = x + ((w - len[curLine] * scale) / 2);
  for (uint16_t c = 0; c < s.length(); c++) {
    char curChar = s.charAt(c);
    if (curChar == '\n') {
      y += gfxFont->yAdvance;
      curLine--;
      pos = x + ((w - len[curLine] * scale) / 2);
      continue;
    }
    b_drawChar(pos, y, curChar, color, bg, scale, scale);
    GFXglyph *glyph = gfxFont->glyph + curChar - gfxFont->first;
