...
// different page size but modifications DEPENDS ON: Arduino SPI library >
...
// for SPI Transactions (Arduino 1.5.8 and above) > Selective merge by Felix
...
/// To use any write commands the WEL bit in the status register must be set
/// to 1. This is accomplished by sending a 0x06 command before any such
/// write/erase command. The WEL bit in the status register resets to the
/// logical �0� state after a device power-up or reset. In addition, the WEL bit
/// will be reset to the logical �0� state automatically under the following
/// conditions: � Write Disable operation completes successfully � Write Status
/// Register operation completes successfully or aborts � Protect Sector
/// operation completes successfully or aborts � Unprotect Sector operation
/// completes successfully or aborts � Byte/Page Program operation completes
/// successfully or aborts � Sequential Program Mode reaches highest unprotected
/// memory location � Sequential Program Mode reaches the end of the memory
/// array � Sequential Program Mode aborts � Block Erase operation completes
/// successfully or aborts � Chip Erase operation completes successfully or
/// aborts � Hold condition aborts
...
#define SPIFLASH_CHIPERASE                                                     \
  0x60 // chip erase (may take several seconds depending on size)
       // but no actual need to wait for completion (instead need to check the
       // status register BUSY bit)
#define SPIFLASH_STATUSREAD 0x05  // read status register
#define SPIFLASH_STATUSWRITE 0x01 // write status register
#define SPIFLASH_ARRAYREAD                                                     \
  0x0B // read array (fast, need to add 1 dummy byte after 3 address bytes)
...
#define SPIFLASH_IDREAD                                                        \
  0x9F // read JEDEC manufacturer and device ID (2 bytes, specific bytes for
       // each manufacturer and device) Example for Atmel-Adesto 4Mbit
       // AT25DF041A: 0x1F44 (page 27:
...
public:
  ...
  void readBytes(uint32_t addr, void *buf, uint32_t len);
  void writeByte(uint32_t addr, uint8_t byt);
  void writeBytes(uint32_t addr, const void *buf, uint32_t len);
  bool busy();
  void chipErase();
