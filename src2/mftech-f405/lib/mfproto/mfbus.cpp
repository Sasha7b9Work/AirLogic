...
void mfBus::setName(const char *_name) { strncpy(name, _name, 31); }
...
bool mfBus::slaveExists(uint8_t slaveId) {
  bool ret = false;
  for (uint8_t devs = 0; devs < getSlavesCount(); devs++) {
    if (getSlave(devs).device == slaveId) {
      ret = true;
      break;
    }
  }
